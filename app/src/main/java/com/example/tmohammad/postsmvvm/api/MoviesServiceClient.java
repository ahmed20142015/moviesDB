...
public class MoviesServiceClient {
    ...
    private static final String API_TOKEN = "f5e69138cb72c2280adc6636749ede96";
     ...
     *                     API Call
     */
    public static void getMoviesByname (MovieService service, String query, int page, int itemsPerPage, final ApiCallback<Movie> apiCallback) {
        ...
        service.getMoviesByName(API_TOKEN , page , query).enqueue(new Callback<MoviesResponce>() {
    ...
    public static void getMovieReview (MovieService service, Integer movieId, int page , final ApiCallback<Review> apiCallback) {
        Log.d(LOG_TAG, String.format("getMovieReview: query: %d, page: %s", movieId, page));

        service.getMovieReview(API_TOKEN , movieId , page).enqueue(new Callback<ReviewsResponce>() {
            //Called when the response is received
            @Override
            public void onResponse(@Nullable Call<ReviewsResponce> call, @NonNull Response<ReviewsResponce> response) {
                Log.d(LOG_TAG, "onResponse: Got response: " + response);
                if (response.isSuccessful()) {
                    //Retrieving the movie Items when the response is successful
                    List<Review> items;
            ...
            //Called on Failure of the request
            @Override
            public void onFailure(@Nullable Call<ReviewsResponce> call, @NonNull Throwable t) {
                Log.d(LOG_TAG, "onFailure: Failed to get data");
                //Pass the error to the callback
                apiCallback.onError(t.getMessage() != null ?
                        t.getMessage() : "Unknown error");
